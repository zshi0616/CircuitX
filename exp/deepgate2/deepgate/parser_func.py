...
def parse_pyg_mlpgate(x, edge_index, tt_dis, tt_pair_index, \
    ...
    forward_level = torch.tensor(forward_level, dtype=torch.long)
    backward_level = torch.tensor(backward_level, dtype=torch.long)
    forward_index = torch.LongTensor([i for i in range(len(x_torch))])
